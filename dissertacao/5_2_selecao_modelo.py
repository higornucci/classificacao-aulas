...
conjunto_treinamento = pd.read_csv('../input/DadosCompletoTransformadoMLBalanceado.csv', encoding='utf-8',
                                   delimiter='\t')
conjunto_teste = pd.read_csv('../input/DadosCompletoTransformadoMLTeste.csv', encoding='utf-8', delimiter='\t')
...
# conjunto_teste.to_csv('../input/DadosCompletoTransformadoMLTeste.csv', sep='\t')
...
def gerar_matriz_confusao(modelo, tipo, X_treino, Y_treino):
    y_pred = cross_val_predict(modelo, X_treino, Y_treino, cv=kfold)
...
def rodar_algoritmos():
    ...
    cv_results_balanced = rodar_modelo(modelo, nome, 'Balanceado', X_treino, Y_treino)
