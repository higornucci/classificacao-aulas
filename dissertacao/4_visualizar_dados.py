...
train_set, test_set = train_test_split(dados_completo, test_size=0.1, random_state=42)
print(len(train_set), "train +", len(test_set), "test")

ax = plt.axes()
sns.countplot(x='acabamento', data=test_set, ax=ax)
ax.set_title('Distribuição do acabamento')
plt.show()

# f, axarr = plt.subplots(4, 2, figsize=(15, 15))
#
# sns.boxplot(x='maturidade', y='acabamento', data=test_set, showmeans=True, ax=axarr[0, 0])
# sns.boxplot(x='peso', y='acabamento', data=dados_completo, showmeans=True, ax=axarr[0, 1])
# sns.boxplot(x='questionario_classificacao_estabelecimento_rural', y='acabamento', data=dados_completo, showmeans=True, ax=axarr[1, 0])
...
# sns.boxplot(x='area_manejada_80_boa_cobertura_vegetal', y='acabamento', data=dados_completo, showmeans=True, ax=axarr[3, 0])

# axarr[0, 0].set_title('Maturidade')
# axarr[0, 1].set_title('Peso')
